...
int TicTacToe::makeMove() {
  ...
  for (int r = 0; r < 3; r++) {
    if (board[r][0] != 0 && board[r][0] == board[r][1] && board[r][0] == board[r][2] && board[r][1] == board[r][2]) {
      if (board[r][0] == 1) {return PLAYER1;}
      else {return PLAYER2;}
    }
    if (board[0][r] != 0 && board[0][r] == board[1][r] && board[0][r] == board[2][r] && board[1][r] == board[2][r]) {
      if (board[0][r] == 1) {return PLAYER1;}
      else {return PLAYER2;}
    }
  }
  if (board[0][0] != 0 && board[0][0] == board[1][1] && board[0][0] == board[2][2] && board[1][1] == board[2][2]) {
    if (board[0][0] == 1) {return PLAYER1;}
    else {return  PLAYER2;}
  }
  if (board[2][0] != 0 && board[2][0] == board[1][1] && board[2][0] == board[0][2] && board[1][1] == board[0][2]) {
  ...
  for (int row = 0; row < 3; row++) {
    for (int col = 0; col < 3; col++) {
      if (board[row][col] == 0) {
	return KEEP_PLAYING; //there is some square that is empty
      }
    }
  }
  return DRAW;
